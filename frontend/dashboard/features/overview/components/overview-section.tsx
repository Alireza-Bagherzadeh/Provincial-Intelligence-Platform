...
export function OverviewSection({ data, onNavigate }: { data: CommandCenterData; onNavigate: (section: CommandSectionId) => void }) {
  ...
  return <>
    ...
    <GovernancePanel onNavigate={onNavigate} />

    <section className="analytics-grid equal overview-analytics">
      <article className="card analytics-card"><div className="card-header"><div><h2>نرخ پاسخ‌گویی شهرستانی</h2><p className="muted">پاسخ‌داده‌شده در برابر کل درخواست‌های مردمی</p></div><button type="button" className="text-button" onClick={() => onNavigate("citizen")}>جزئیات</button></div><LineAreaChart values={citizenRates.length ? citizenRates : [0, 0]} labels={citizenLabels} /></article>
      <article className="card analytics-card"><div className="card-header"><div><h2>ترکیب وضعیت پروژه‌ها</h2><p className="muted">نمای سریع سبد پروژه‌ها</p></div><button type="button" className="text-button" onClick={() => onNavigate("projects")}>پروژه‌ها</button></div><DonutChart centerLabel="پروژه" segments={[{ label: "مطابق برنامه", value: projectCounts.on_track, tone: "success" }, { label: "نیازمند توجه", value: projectCounts.attention, tone: "warning" }, { label: "بحرانی", value: projectCounts.critical, tone: "danger" }, { label: "تکمیل", value: projectCounts.complete, tone: "cyan" }]} /></article>
    </section>

    <section className="analytics-grid two-one overview-analytics">
      <article className="card analytics-card"><div className="card-header"><div><h2>نبض محورهای راهبردی استان</h2><p className="muted">مقدار فعلی در برابر معیار مرجع</p></div><button type="button" className="text-button" onClick={() => onNavigate("sectors")}>بخش‌بندی هوشمند</button></div><HorizontalBarChart rows={provinceSectors.map((item) => ({ label: item.domain, value: Number(item.value), benchmark: item.benchmarkValue ? Number(item.benchmarkValue) : undefined, status: item.status }))} /></article>
      <article className="card analytics-card"><div className="card-header"><div><h2>پروفایل عملکرد دستگاه</h2><p className="muted">{topOrg?.name ?? "بدون داده"}</p></div><button type="button" className="text-button" onClick={() => onNavigate("performance")}>ارزیابی عملکرد</button></div>{orgKpis.length >= 3 ? <RadarChart values={orgKpis.map((item) => Number(item.score))} labels={orgKpis.map((item) => item.label)} /> : <div className="empty-state">شاخص‌های جزئی کافی وجود ندارد.</div>}</article>
    </section>

    <section className="analytics-grid two-one overview-analytics">
      <article className="card analytics-card"><div className="card-header"><div><h2>پیش‌بینی و هشدار زودهنگام</h2><p className="muted">مقدار فعلی در برابر پیش‌بینی همراه با بازه عدم‌قطعیت</p></div><button type="button" className="text-button" onClick={() => onNavigate("forecast")}>مرکز پیش‌بینی</button></div><ForecastBandChart rows={forecastRows} /></article>
      <article className="card analytics-card"><div className="card-header"><div><h2>فشار رخدادهای فعال</h2><p className="muted">میانگین امتیاز اثر بحران‌های باز یا در حال پایش</p></div><button type="button" className="text-button" onClick={() => onNavigate("crisis")}>مرکز بحران</button></div><GaugeChart value={crisisImpact} label="اثر بحران" /></article>
    </section>

    <section className="analytics-grid equal overview-analytics">
      <CountyPortfolioCard counties={data.counties} onOpenComparison={() => onNavigate("benchmark")} />
      <article className="card analytics-card"><div className="card-header"><div><h2>مهم‌ترین هشدارهای زودهنگام</h2><p className="muted">ترکیب رخداد و پیش‌بینی برای توجه مدیریتی</p></div></div><div className="alert-list">{activeCrises.slice(0, 4).map((crisis) => <div className={`alert-row ${crisis.severity === "critical" ? "critical" : "attention"}`} key={crisis.id}><i /><div><h3>{crisis.title}</h3><p>{crisis.county?.name ?? "استانی"} · اثر {crisis.impactScore}/۱۰۰</p></div><span>{crisis.status === "open" ? "باز" : "پایش"}</span></div>)}</div></article>
    </section>

    <section className="grid overview-grid">
      <ExecutiveBrief entries={data.brief} />
      <article className="card news-pulse"><div className="card-header"><div><h2>خبر و افکار عمومی</h2><p className="muted">آرشیو مستند و لایه خبرگزاری هوشمند</p></div><button type="button" className="text-button" onClick={() => onNavigate("news")}>رصد خبر</button></div><div className="news-pulse-top"><strong>{data.newsArticles.length}</strong><span>محتوای رصدشده</span></div><SentimentStack positive={positive} neutral={neutral} negative={negative} /><div className="sentiment-key"><span><i className="positive" />مثبت {positive}</span><span><i className="neutral" />خنثی {neutral}</span><span><i className="negative" />منفی {negative}</span></div><div className="importance-row"><span>اهمیت محتوای اخیر</span><SparkBars values={data.newsArticles.slice(0, 10).map((item) => item.importance)} /></div></article>
